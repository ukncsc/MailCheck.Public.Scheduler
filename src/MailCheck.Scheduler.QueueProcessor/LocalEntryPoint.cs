...
using MailCheck.Scheduler.QueueProcessor.Seeding;
using Microsoft.Extensions.CommandLineUtils;

namespace MailCheck.Scheduler.QueueProcessor
{
    public static class LocalEntryPoint
    {
        public static void Main(string[] args)
        {
            ...
            {
                CommandOption userConnectionString = command.Option("-d", "User Connection String", CommandOptionType.SingleValue);
                CommandOption queueUrl = command.Option("-q", "Scheduler Queue Processor Queue Url", CommandOptionType.SingleValue);
                CommandOption serviceName = command.Option("-s", "Service Name", CommandOptionType.SingleValue);


                command.OnExecute(() =>
                {
                    ISeeder seeder = SeederFactory.Create(userConnectionString.Value(), queueUrl.Value(), serviceName.Value());

                    seeder.SeedCreateScheduledReminder().GetAwaiter().GetResult();
        ...
        private static async Task RunLambda()
        {
            ...
            LambdaEntryPoint dmarcPollerLambdaEntryPoint = new LambdaEntryPoint();
